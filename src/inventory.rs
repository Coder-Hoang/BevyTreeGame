...
impl Inventory {
            ...
            .all(|(item, c)| self.items.get(&item).map_or(false, |count| * count >= c))
        {
            for (item, count) in items {
                let c = self
                    .items
                    .get_mut(&item)
                    .expect("We already check that this is the map.");
                *c -= count;
                if *c == 0 {
                    self.items.remove(&item);
                }
    ...
    pub fn get_items_count(&self, item: Item) -> u32 {
    ...
    commands
        .spawn(NodeBundle {
            style: Style {
                ...
                flex_duration: FlexDirection :: Column,
                ..default()
            },
            ..default()
        })
...
fn update_inventory_ui (
    ...
    for (mut text, mut style, material) in material_text.iter_mut() {
        ...
        if count > 0 {
            ...
            text_sections[0].value = format!("{}, {}", material.0, count);
        } else {
