...
use crate :: {
    ...
    pickup :: {OnPickUpEvent, PickupTag},
};
...
fn perform_pickup(
    ...
    for evvent in pickup_events.read() {
        let Ok(item) = item_pickup.get(event.pickup_entity) else {
            continue;
        };
        let Ok((mut receiver, sound)) = receivers.get_mut(event.reciever_entity) else {
            continue
        };
...
fn setup_item_pickup_resources(mut commands: Commands, asset_server: Res<AssetServer>) {
    commands.insert_resource(ItemModels(HashMap :: from_iter([
        ...
        (
            Item :: Banada,
            vec![asset_server.load("models/items/banada_model.gltf#Scene0")],
        ),
        (
            Item :: Apple,
