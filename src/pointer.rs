...
use bevy_vector_shapes :: {prelude :: ShapePainter, shape :: RectPainter};
...
pub fn update_pointer_pos (
    ...
    pointer.pointer_om = window.cursor_position().and_then(|cursor|{
        let ray = camera.viewport_to_world(camera_t, cursor)?;

        let mut filter = QueryFilter :: default();
        filter.groups = Some(CollisionGroups :: new (
            Groups :: from_bits(COLLISION_POINTER | COLLISION_PROJECTILES).unwrap(),
            Groups :: from_bits(COLLISION_POINTER | COLLISION_CHARACTER).unwrap(),
        ));
        let (collider_entity, _) = rapier.cast_ray(ray.origin, ray.direction, f32 :: MAX, true, filter)?;
        let entity = q_parent
