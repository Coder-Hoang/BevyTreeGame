...
use crate :: {
    ...
    tree :: TreeTrunkTag,
    waves :: {WaveDescriptors, WaveDescriptorAsset},
    weapon :: WeaponType,
};
...
fn check_for_no_robots(player: Query<&Body>) -> bool {
    players
        .into_iter()
        .filter(|b| {
...
pub fn handle_next_wave (
    ...
    mut spawn_player_event: EventWritter<SpawnPlayerEvent>,
    mut notification_event: EventWritter<NotificationEvent>,
    mut spawn_shop_item_event: EventWritter<SpawnShopEvent>,
    wave_descriptors: Res<WaveDescriptors>,
    wave_descriptor_assets: Res<Assets<WaveDesscriptorsAsset>>,
) {
    let AppState :: Wave(wave) = app_sate.as_mut() else {
    ...
    *wave += 1;
    ...
    let wave_descirptors = &wave_descriptor_assets.get(&wave_descirptors.0).unwrap().0;
    let is_last_wave = wave_descirptors.len() - 1 == *wave;
    let wave_descritor = wave_descriptors[*wave].clone();
...
pub fn handle_los (
    ...
    commands.spawn(AudioBundle {
        sourse: asset_server.load("sounds/lost.ogg"),
        ..default()
    });
    ...
    commands.insert_resoure(AppState :: Lost);
}