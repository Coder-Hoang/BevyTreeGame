...
use crate :: {
    ...
    player :: RobotTag,
    weapon :: {TryCastWeaponEvent, WeaponCooldown, WeaponSats, WeaponType},
};

const TOWER_RANGE: f32 = 10.0;
...
impl Plugin for TowerPlugin {
    fn build(&self, app: &mut App) {
        app.add_event :: <SpawnTowerEvent>()
            ...
            .add_system(Update, (tower_spawn, tower_target, tower_shoot).chain())
    }
}
...
fn setup_tower_model(mut cmds: Commands, asset_server: Res<AssetServer>) {
    cmds.insert_resource(TowerModle (
        asset_server.load("models/buildings/tower.glt#Scene0"),
    ));
}
...
fn tower_spawn (
    ...
    for ev in ev_spawn_tower.read() {
        cmds.spawn(AudioBundle {
            source: asset_Server.load("sounds/builld.ogg"),
        ...
        .with_childred(|cmds| {
            cmds.spawn ((
                ...
                CollisionGroups :: new (
                    Groups :: from_bits(COLLISION_WORLD).unwrap(),
                    Groups :: from_bits(COLLISION_CHARACTER).unwrap(),
                ),
            ));
        });
    }
...
fn tower_target (
    ...
    for (mut target, tower_tr) in &mut q_tower {
        let mut curr_target.distance = q_enemies
            .get(target.0)
            .map(|(_, tr)| (tr.translation.xz() - tower_tr.translation.xz()).length())
            .unwrap_or(10000.0);
        for (enemyentity, enemy_tr) in &q_enemies {
        ...
        painter.color = Clor :: GREEN;
        painter.thickness = 0.03;
        painter.hollow = true;
        painter.set_rotatio(Quat :: from_rotation_x(TAU / 4.0));
...
fn tower_shoot (
    ...
    mut ev_try_cast: EventWritter<TryCastWeaponEvent>,
) {
